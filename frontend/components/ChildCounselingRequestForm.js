...
export default function ChildCounselingRequestForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await submitForm({ service: 'child-counseling', ...form, preferredDate: form.date });
      setStatus('success');
    } catch (err) {
  ...
  return (
    <form className="ca-req-form" onSubmit={handleSubmit}>
      <label className="ca-req-label">Full Name</label>
      <div className="ca-req-input-wrap">
        <IconUserCircle />
        <input name="name" value={form.name} onChange={handleChange} placeholder="Your name" required />
      </div>

      <label className="ca-req-label">Email Address</label>
      <div className="ca-req-input-wrap">
        <IconMail />
        <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="you@example.org" required />
      </div>

      <label className="ca-req-label">Phone Number</label>
      <div className="ca-req-input-wrap">
        <IconPhone />
        <input name="phone" value={form.phone} onChange={handleChange} placeholder="(123) 456-7890" />
      </div>

      <label className="ca-req-label">Preferred Date</label>
      <div className="ca-req-input-wrap">
        <IconCalendar />
        <input name="date" type="date" value={form.date} onChange={handleChange} placeholder="mm/dd/yyyy" />
      </div>

      <label className="ca-req-label">Select Your Insurance Provider</label>
      <div className="ca-req-input-wrap">
        <IconShield />
      ...
      <label className="ca-req-label">Tell us more about your needs</label>
