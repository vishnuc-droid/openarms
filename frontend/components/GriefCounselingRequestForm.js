...
export default function GriefCounselingRequestForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await submitForm({ service: 'grief-counseling', ...form, preferredDate: form.date });
      setStatus('success');
    } catch (err) {
  ...
  if (status === 'success') {
    return (
      <div className="grief-req-success">
  ...
  return (
    <form className="grief-req-form" onSubmit={handleSubmit}>
      <label className="grief-req-label">Full Name</label>
      <div className="grief-req-input-wrap">
        <IconUserCircle />
        <input name="name" value={form.name} onChange={handleChange} placeholder="Your name" required />
      </div>

      <label className="grief-req-label">Email Address</label>
      <div className="grief-req-input-wrap">
        <IconMail />
        <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="you@example.org" required />
      </div>

      <label className="grief-req-label">Phone Number</label>
      <div className="grief-req-input-wrap">
        <IconPhone />
        <input name="phone" value={form.phone} onChange={handleChange} placeholder="(123) 456-7890" />
      </div>

      <label className="grief-req-label">Preferred Date</label>
      <div className="grief-req-input-wrap">
        <IconCalendar />
        <input name="date" type="date" value={form.date} onChange={handleChange} placeholder="mm/dd/yyyy" />
      </div>

      <label className="grief-req-label">Select Your Insurance Provider</label>
      <div className="grief-req-input-wrap">
        <IconShield />
      ...
      <label className="grief-req-label">Tell us more about your needs</label>
      ...
      {error && <p className="grief-req-error">{error}</p>}

      <button type="submit" className="grief-btn grief-req-btn-primary grief-req-submit" disabled={status === 'submitting'}>
        <IconSend />
        {status === 'submitting' ? 'Sending…' : 'Request Appointment'}
      </button>

      <p className="grief-req-privacy">
        <IconLock />
