...
export default function AdultCounselingRequestForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await submitForm({ service: 'adult-counseling', ...form, preferredDate: form.date });
      setStatus('success');
    } catch (err) {
  ...
  if (status === 'success') {
    return (
      <div className="adult-req-success">
  ...
  return (
    <form className="adult-req-form" onSubmit={handleSubmit}>
      <label className="adult-req-label">Full Name</label>
      <div className="adult-req-input-wrap">
        <IconUserCircle />
        <input name="name" value={form.name} onChange={handleChange} placeholder="Your name" required />
      </div>

      <label className="adult-req-label">Email Address</label>
      <div className="adult-req-input-wrap">
        <IconMail />
        <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="you@example.org" required />
      </div>

      <label className="adult-req-label">Phone Number</label>
      <div className="adult-req-input-wrap">
        <IconPhone />
        <input name="phone" value={form.phone} onChange={handleChange} placeholder="(123) 456-7890" />
      </div>

      <label className="adult-req-label">Preferred Date</label>
      <div className="adult-req-input-wrap">
        <IconCalendar />
        <input name="date" type="date" value={form.date} onChange={handleChange} placeholder="mm/dd/yyyy" />
      </div>

      <label className="adult-req-label">Select Your Insurance Provider</label>
      <div className="adult-req-input-wrap">
        <IconShield />
      ...
      <label className="adult-req-label">Tell us a bit about what&rsquo;s going on</label>
      ...
      {error && <p className="adult-req-error">{error}</p>}

      <button type="submit" className="adult-btn adult-req-btn-primary adult-req-submit" disabled={status === 'submitting'}>
        <IconSend />
        {status === 'submitting' ? 'Sending…' : 'Request Appointment'}
      </button>

      <p className="adult-req-privacy">
        <IconLock />
