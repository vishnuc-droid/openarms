...
export default function ProBonoRequestForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await submitForm({ service: 'pro-bono-counseling', ...form, preferredDate: form.date });
      setStatus('success');
    } catch (err) {
  ...
  return (
    <form className="fs-req-form" onSubmit={handleSubmit}>
      ...
      <label className="fs-req-label">Tell us a bit about your situation</label>
      ...
      <button type="submit" className="fs-btn fs-req-btn-primary fs-req-submit" disabled={status === 'submitting'}>
        <IconSend />
        {status === 'submitting' ? 'Sending…' : 'Ask About Pro Bono Counseling'}
      </button>
