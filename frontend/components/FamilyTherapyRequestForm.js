...
export default function FamilyTherapyRequestForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await submitForm({ service: 'family-therapy', ...form, preferredDate: form.date });
      setStatus('success');
    } catch (err) {
  ...
  if (status === 'success') {
    return (
      <div className="family-req-success">
  ...
  return (
    <form className="family-req-form" onSubmit={handleSubmit}>
      <label className="family-req-label">Full Name</label>
      <div className="family-req-input-wrap">
        <IconUserCircle />
        <input name="name" value={form.name} onChange={handleChange} placeholder="Your name" required />
      </div>

      <label className="family-req-label">Email Address</label>
      <div className="family-req-input-wrap">
        <IconMail />
        <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="you@example.org" required />
      </div>

      <label className="family-req-label">Phone Number</label>
      <div className="family-req-input-wrap">
        <IconPhone />
        <input name="phone" value={form.phone} onChange={handleChange} placeholder="(123) 456-7890" />
      </div>

      <label className="family-req-label">Preferred Date</label>
      <div className="family-req-input-wrap">
        <IconCalendar />
      ...
      {error && <p className="family-req-error">{error}</p>}

      <button type="submit" className="family-btn family-req-btn-primary family-req-submit" disabled={status === 'submitting'}>
        <IconSend />
