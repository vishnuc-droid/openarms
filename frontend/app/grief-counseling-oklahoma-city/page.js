...
export default function GriefCounselingPage() { return <main className="grief-page">
  ...
  <ScrollReveal />
  <section className="grief-hero-outer"><div className="grief-hero-bg"><img src="/mg5.png" alt="Woman finding a quiet moment while processing grief with her counselor" /><div className="grief-container grief-hero-content"><p className="grief-kicker">YOU DON&rsquo;T HAVE TO GRIEVE ALONE.</p><h1>Grief Counseling<br />in Oklahoma City</h1><i /><p className="grief-lead">There Is No Right Way or Right Timeline to Grieve</p><p>At Open Arms Initiative, we provide a compassionate space to process loss, understand what you&apos;re experiencing, and find support as you learn to carry what has changed.</p><div className="grief-actions"><Link href="/contact" className="grief-btn grief-solid">Request an Appointment</Link><a href="#support" className="grief-btn grief-outline">Explore Grief Support</a></div></div></div></section>
  ...
  <section id="approach" className="grief-section"><div className="grief-container"><div className="grief-more"><img className="reveal reveal-from-left" src="/gf3.jpg" alt="Support group having an open, compassionate conversation" /><div className="reveal reveal-from-right"><p className="grief-kicker-label">BEREAVEMENT THERAPY OKC</p><h2>You Don&rsquo;t Have to &ldquo;Move On&rdquo; to Move Forward</h2><p>Grief counseling isn&apos;t about forgetting. It isn&apos;t about forcing yourself to stop feeling sad. And it isn&apos;t about following someone else&apos;s timeline for when you should feel better.</p><p>Through <strong>Bereavement Therapy in OKC</strong>, you can have space to talk about the person, relationship, experience, or future you lost, and explore how that loss has affected your life.</p><p>Counseling may support you as you:</p><ul>{['Process difficult emotions', 'Understand your individual grief experience', 'Adjust to changes following a loss', 'Explore guilt, anger, confusion, or unanswered questions', 'Develop ways to cope with difficult days', 'Navigate changes in relationships and family dynamics', 'Find ways to honor meaningful memories', 'Reconnect with everyday life at your own pace', 'Consider what moving forward means for you'].map(x => <li key={x}>{x}</li>)}</ul><p><strong>Moving forward doesn&apos;t mean leaving what mattered behind.</strong></p></div></div></div></section>
