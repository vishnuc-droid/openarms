...
const PROGRAMS = [
  ...
  {
    img: '/Training/authentic-group-therapy-meeting.webp',
    ...
    <main ref={revealRootRef} className="tmh2-page">
      ...
      <section className="tmh2-hero" ref={heroRef}>
        <div className="tmh2-hero-inner">
          <div className="tmh2-hero-copy reveal reveal-from-left">
            <svg className="tmh2-copy-dots" viewBox="0 0 90 70" fill="none" aria-hidden="true">
              {[0,1,2,3].map((row) => [0,1,2,3,4].map((col) => (
                <circle key={`${row}-${col}`} cx={col*16+8} cy={row*16+8} r="2.4" fill="#8DC540" opacity="0.3" />
              )))}
            </svg>
            ...
            <p className="tmh2-hero-desc">
              A case worker who doesn&apos;t know how to respond when a client shuts down. A volunteer who takes a teenager&apos;s outburst personally. A program director who realizes half her staff are running on empathy fumes. This training gives community and nonprofit teams a foundational, practical understanding of trauma and mental health, the kind that applies across roles, not just one department.
            </p>
          ...
          <div className="tmh2-hero-photo reveal reveal-from-right">
            <svg className="tmh2-hero-arc" viewBox="0 0 520 460" fill="none" aria-hidden="true">
              <path d="M40 200 C 40 80, 140 10, 300 10" stroke="#8DC540" strokeWidth="5" strokeLinecap="round" />
            </svg>
            <svg className="tmh2-hero-dots" viewBox="0 0 90 70" fill="none" aria-hidden="true">
              {[0, 1, 2, 3].map((row) => [0, 1, 2, 3, 4].map((col) => (
                <circle key={`${row}-${col}`} cx={col * 16 + 8} cy={row * 16 + 8} r="2.4" fill="#8DC540" opacity={row === 0 || col === 4 ? 0.55 : 0.3} />
              )))}
            </svg>
            <div className="tmh2-hero-frame">
              <Image
                src="/Training/authentic-group-therapy-meeting (1).webp"
                alt="Nonprofit and community organization staff in a trauma and mental health training session"
                className="tmh2-hero-img"
                width={900}
                height={700}
                priority
              />
...
Community and nonprofit teams rarely walk in with the exact same mix of roles, experience levels, or prior training, so the first conversation is about who's actually going to be in the room and what they need most, not a fixed curriculum. Reach out and we'll talk through your team, your timing, and what would be most useful to cover.
      ...
      <style>{`
        .tmh2-page{
          --green:#8DC540; --green-dark:#093129; --cream:#EDFFE1; --tint:#F3F8ED;
        ...
        .tmh2-page h1,.tmh2-page h2,.tmh2-page h3,.tmh2-page h4,.tmh2-page h5{
          font-family:'DM Sans', Arial, sans-serif; font-weight:700; color:var(--green-dark); margin:0;
        }
        ...
        .tmh2-btn-primary{ background:var(--green); color:var(--green-dark); border-radius:8px; }
        .tmh2-btn-primary:hover{ background:#7ab234; }
        .tmh2-btn-outline{ background:transparent; color:var(--green-dark); border:1.5px solid #c9d6c5; border-radius:8px; }
        .tmh2-btn-outline:hover{ background:var(--tint); border-color:#8DC540; }
        .tmh2-btn-dark{ background:var(--green-dark); color:var(--cream); }
        .tmh2-btn-dark:hover{ opacity:.9; }
        .tmh2-btn-accent{ background:var(--green); color:var(--green-dark); }
        .tmh2-link{ color:var(--green-dark); font-weight:700; font-size:14px; border-bottom:1.5px solid var(--green); }

        /* HERO — full width */
        .tmh2-hero{ padding:44px 0 20px; min-height:100vh; display:flex; align-items:center; }
        .tmh2-hero-inner{ display:grid; grid-template-columns:1.05fr 1fr; gap:56px; align-items:center; max-width:100%; width:100%; padding:0 4vw; }
        .tmh2-pill{ display:inline-flex; align-items:center; gap:8px; background:var(--tint); color:#4c8a1f; border:1px solid rgba(141,197,64,.35); font-size:13px; font-weight:700; padding:8px 16px 8px 14px; border-radius:30px; margin-bottom:20px; }
        .tmh2-hero-h1{ font-size:clamp(2.4rem, 4vw, 3.6rem); line-height:1.12; margin-bottom:18px; }
        .tmh2-hero-h1-accent{ color:var(--green); }
        .tmh2-hero-desc{ font-size:15.5px; color:var(--muted); line-height:1.7; max-width:520px; margin-bottom:26px; }
        .tmh2-hero-ctas{ display:flex; gap:14px; flex-wrap:wrap; }
        .tmh2-hero-photo{ position:relative; padding:34px 20px 30px 10px; }
        .tmh2-hero-arc{ position:absolute; top:-18px; left:-40px; width:60%; height:70%; z-index:0; pointer-events:none; }
        .tmh2-hero-dots{ position:absolute; top:10px; left:-20px; width:110px; height:90px; z-index:0; pointer-events:none; }
        .tmh2-copy-dots{ position:absolute; top:-10px; right:0; width:100px; height:80px; z-index:0; pointer-events:none; }
        .tmh2-hero-copy{ position:relative; }
        .tmh2-hero-leaves{ position:absolute; right:-10px; bottom:-14px; width:150px; height:auto; z-index:2; pointer-events:none; }
        .tmh2-hero-frame{
          position:relative; z-index:1; overflow:hidden;
          border-radius: 45% 45% 45% 4% / 50% 50% 12% 12%;
          box-shadow:0 20px 50px rgba(9,49,41,.14);
        }
        .tmh2-hero-img{ width:100%; height:auto; display:block; object-fit:cover; aspect-ratio:9/8; }
        @media(max-width:1000px){
          .tmh2-hero-arc, .tmh2-hero-dots{ display:none; }
          .tmh2-hero-photo{ padding:0; }
          .tmh2-hero-frame{ border-radius:26px; }
        }
        ...
        .tmh2-cred{ flex:1; min-width:220px; display:flex; align-items:center; gap:16px; padding:30px 32px; font-weight:600; font-size:16px; color:var(--green-dark); border-right:1px solid var(--line); }
        ...
        .tmh2-who-card:hover{ transform:translateY(-5px); box-shadow:0 14px 32px rgba(9,49,41,0.12); border-color:rgba(141,197,64,.45); }
        ...
        .tmh2-who-card h4{ font-size:16px; font-weight:700; margin-bottom:8px; color:var(--green-dark); line-height:1.35; }
        ...
        /* HELPS GRID */
        .tmh2-helps-grid{ display:grid; grid-template-columns:repeat(4,1fr); gap:18px; }
        ...
        /* FLAGSHIP */
        .tmh2-flagship-section{ background:var(--green-dark); padding-left:0 !important; padding-right:0 !important; }
        ...
        .tmh2-flagship p{ color:#c9dcc3; font-size:14.5px; line-height:1.6; margin:0; }
        .tmh2-flagship-cta{ flex-shrink:0; }

        /* PROGRAMS */
        .tmh2-programs-grid{ display:grid; grid-template-columns:repeat(5,1fr); gap:16px; }
        ...
        .tmh2-program-card h4{ font-size:15px; font-weight:700; margin-bottom:6px; line-height:1.3; color:var(--green-dark); }
        ...
        .tmh2-jamie-content > p{ color:#c9dcc3; font-size:14.5px; line-height:1.7; margin-bottom:18px; max-width:600px; }
        ...
        .tmh2-jamie-video-sub{ color:#a8c99a; font-size:12.5px; margin:0; }

        /* FORMATS — elegant dark "journey" band */
        .tmh2-formats-section{ position:relative; overflow:hidden; isolation:isolate; background:var(--green-dark); padding:70px 0; }
        .tmh2-formats-glow{ position:absolute; top:-140px; right:-100px; width:420px; height:420px; border-radius:50%; background:radial-gradient(circle, rgba(141,197,64,0.18) 0%, transparent 70%); pointer-events:none; }
        .tmh2-formats-header{ text-align:center; max-width:620px; margin:0 auto 50px; }
        .tmh2-formats-eyebrow{ display:inline-block; font-size:12.5px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:var(--green); background:rgba(141,197,64,0.12); border:1px solid rgba(141,197,64,0.3); padding:6px 16px; border-radius:20px; margin-bottom:16px; }
        .tmh2-formats-title{ color:#ffffff; font-size:30px; margin-bottom:12px; }
        .tmh2-formats-sub{ color:#d4e8d4; font-size:14.5px; line-height:1.65; margin:0; }
        .tmh2-formats-track{ position:relative; display:grid; grid-template-columns:repeat(6,1fr); gap:12px; }
        .tmh2-formats-line{ position:absolute; top:29px; left:8%; right:8%; height:1.5px; background:repeating-linear-gradient(90deg, rgba(141,197,64,0.4) 0 8px, transparent 8px 16px); z-index:0; }
        ...
        .tmh2-why-tile:hover{ transform:translateY(-4px); box-shadow:0 16px 34px rgba(9,49,41,0.1); border-color:rgba(141,197,64,.4); }
        ...
        .tmh2-why-tile--feature .tmh2-why-tile-icon{ background:rgba(141,197,64,0.15); color:var(--green); width:64px; height:64px; }
        .tmh2-why-tile--feature h4{ color:var(--white); font-size:20px; }
        .tmh2-why-tile--feature p{ color:#c9dcc3; font-size:14.5px; }
        .tmh2-why-tile--feature:hover{ transform:translateY(-4px); box-shadow:0 20px 40px rgba(9,49,41,0.3); border-color:var(--green); }

/* FINAL CTA */
        .tmh2-final-cta{ background:var(--green-dark); padding:56px 0; }
        ...
        .tmh2-final-cta-inner p{ color:#a8d890; font-size:15px; margin:0; max-width:460px; }

        /* STICKY */
        .tmh2-sticky-bar{ display:none; position:fixed; bottom:0; left:0; right:0; z-index:200; padding:.85rem 1.25rem calc(.85rem + env(safe-area-inset-bottom,0px)); background:rgba(9,49,41,.97); backdrop-filter:blur(8px); transform:translateY(100%); transition:transform .3s cubic-bezier(.22,1,.36,1); }
        ...
        @media(max-width:640px){
          ...
          .tmh2-programs-grid{ grid-template-columns:1fr 1fr; }
          ...
          .tmh2-formats-track{ grid-template-columns:repeat(2,1fr); }
          .tmh2-why-bento{ grid-template-columns:1fr; grid-template-rows:auto; }
          .tmh2-why-tile--mid{ grid-column:auto; grid-row:auto; }
          .tmh2-why-tile--feature{ grid-column:auto; grid-row:auto; }
          .tmh2-intro-banner{ padding:36px 16px; gap:28px; }
          .tmh2-section-title{ font-size:26px; }
